...
export const withTableCellHTMLDeserializerTransform: HTMLDeserializerWithTransform = (next, serializer) => {
  return (node, options = {}) => {
    const { text } = options
    if (isDOMHTMLElement(node) && node.nodeName === 'TD') {
      const children: Descendant[] = []
      for (const child of node.childNodes) {
        const content = serializer.transform(child, { text, matchNewline: true })
      ...
      const cell: TableCell = { type: TABLE_CELL_KEY, children, colspan: colSpan, rowspan: rowSpan }
      return [cell]
    }
