import { Editable, ElementAttributes, useIsomorphicLayoutEffect, useLocale, useNodeFocused, useReadOnly } from '@everynote/editor'
import { Editor, Range, Transforms } from '@everynote/models'
import { Popover, PopoverContent, PopoverPortal, PopoverTrigger, Icon, Button } from '@everynote/ui'
...
export const LinkComponent = forwardRef<HTMLAnchorElement, LinkComponentProps & React.AnchorHTMLAttributes<HTMLAnchorElement>>(
  ({ children, element, editor, ...props }, ref) => {
    const { href } = element

    const defaultText = element.children.map((node) => TextSerializer.transformWithEditor(editor, node)).join('')
          ...
          <PopoverContent align="start" sideOffset={5}>
            <div tw="shadow-md rounded px-4 py-2 border border-gray-300 bg-white text-base">
              <div tw="flex gap-2 mb-2 items-center">
                ...
                <StyledInput value={text} onChange={(e) => setText(e.target.value)} type="text" placeholder={locale.textPlaceholder} />
              </div>
              <div tw="flex gap-2 mb-2 items-center">
                <label>{locale.link}</label>
                <StyledInput value={url} onChange={(e) => setUrl(e.target.value)} type="url" placeholder={locale.linkPlaceholder} />
              </div>
