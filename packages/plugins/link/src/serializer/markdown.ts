...
export const withLinkMarkdownSerializerTransform: MarkdownSerializerWithTransform = (next, self) => {
  return (node, options = {}) => {
    if (Link.isLink(node)) {
      return [
        {
          type: 'link',
          url: node.href,
          children: node.children.map((child) => self.transform(child, options)).flat() as StaticPhrasingContent[],
        },
      ]
    }
