...
export const ImageComponent = forwardRef<HTMLImageElement, ImageComponentProps>(
  ({ children, element, editor, ...props }, ref) => {
    const { url, state, rotate, errorMessage, percentage, width, height } =
      element
    ...
    const [imageDOMElement, setImageDOMElement] =
    ...
    useIsomorphicLayoutEffect(() => {
      if (src) {
        readImageElement(src)
          ...
          .catch((err) => {
            if (state === 'done')
              setErrorMsg(typeof err === 'object' ? err.message : err)
          })
      }
    }, [src, state])
    ...
    const handleImageClick = (event: React.MouseEvent) => {
      if (
        isFocusedMouseDownBefore.current &&
    ...
    const { style: styleLocale, viewer: viewerLocale } =
      useLocale<ImageLocale>('image')
      ...
      >
            ...
            >
              ...
              {focused && !readOnly && !isError && (
                <Resizer
                  ...
                  previewImage={
                    isDone && loaded ? rotatedUrl || src : undefined
                  }
        ...
        <PopoverContent autoUpdate={true} side="top" sideOffset={5}>
          <Toolbar mode="inline">
            {options.allowRotate !== false && (
              <Tooltip
                content={viewerLocale.rotateLeft}
                side="top"
                sideOffset={5}
                arrow={false}
              >
