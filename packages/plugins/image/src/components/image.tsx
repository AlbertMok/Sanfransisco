import { Editable, ElementAttributes, useLocale, useNodeFocused, useReadOnly } from '@everynote/editor'
...
export const ImageComponent = forwardRef<HTMLImageElement, ImageComponentProps>(({ children, element, editor, ...props }, ref) => {
    ...
    return (
      <div tw="absolute left-0 top-0 w-full h-full flex justify-center items-center text-primary">
        {percentage !== undefined && (
          ...
          >
            <div tw="absolute left-0 top-0 h-full bg-primary animate-pulse" style={{ width: `${percentage}%` }}></div>
          </div>
        )}
  ...
  const renderImage = () => {
    ...
    return (
      <img
        ...
        alt=""
        draggable={false}
        css={[tw`inline-block align-baseline rounded w-full h-full`, isUploading && tw`opacity-30`]}
        src={rotatedUrl || src}
      />
    )
  }
  ...
  const handleResizeChange = (width: number, height: number) => {
    Transforms.setNodes<Image>(editor, { width, height }, { at: Editable.findPath(editor, element) })
  }
  ...
  const renderStyleStroke = () => {
    ...
    return <div tw="absolute inset-0 w-full h-full border rounded border-gray-300 pointer-events-none" />
  }
  ...
  return (
    <Popover open={isDone && !readOnly ? popoverOpen : false} onOpenChange={handlePopoverOpenChange} trigger="hover">
