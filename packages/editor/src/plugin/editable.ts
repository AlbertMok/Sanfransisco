...
export const Editable = {
    ...
    findPath(editor: Editor, node: Node): Path {
        ...
        throw new Error(
            `Unable to find the path for Editor node: ${Scrubber.stringify(
                node
            )}`
        )
    },
    ...
    toDOMNode(editor: Editor, node: Node): HTMLElement {
        ...
        if (!offsetNode) {
            throw new Error(
                `Cannot resolve a DOM node from Editor node: ${Scrubber.stringify(
                    node
                )}`
            )
        }
    ...
    toDOMPoint(editor: Editor, point: Point): DOMPoint {
        ...
        if (!domPoint) {
            throw new Error(
                `Cannot resolve a DOM point from Editor point: ${Scrubber.stringify(
                    point
                )}`
            )
        }
    ...
    toDOMRange(editor: Editor, range: Range): DOMRange {
        ...
        const isStartAtZeroWidth = !!startEl.getAttribute(
            DATA_EDITABLE_ZERO_WIDTH
        )
        const endEl = (
    ...
    toEditorNode(editor: Editor, offsetNode: DOMNode): Node {
        ...
        if (!node) {
            throw new Error(
                `Cannot resolve a Editor node from DOM node: ${domEl}`
            )
        }
    ...
    findClosestPoint(
        ...
        if (!element) {
            ...
            if (Text.isText(node) || Editor.isVoid(editor, node)) {
                ...
                if (!Editor.isSolidVoid(editor, node)) {
                    ...
                    const adjacent = (reverse ? Editor.previous : Editor.next)(
                        editor,
                        {
                    ...
                    const nodes = Editor.nodes(editor, {
                        at: Editable.findPath(editor, node),
                        match: (n) =>
                            (isGrid && Editor.isGridCell(editor, n)) ||
                            Text.isText(n),
                        mode: 'highest',
                    })
        ...
        if (isDOMNode(nodes)) {
            ...
            if (nearbyLeft && nearbyBelow) {
                ...
                else if (left >= nearbyLeft.rect.right)
                    left = nearbyLeft.rect.right
                else {
                ...
                else if (left >= nearbyRight.rect.right)
                    left = nearbyRight.rect.right
                else {
        ...
        if (Text.isText(node)) {
            ...
            if (!point)
                return Editor.start(editor, Editable.findPath(editor, node))
            return point
        }
        return null
    },
    ...
    findEventPoint(editor: Editor, event: any): Point | null {
        ...
        if (x == null || y == null) {
            throw new Error(
                `Cannot resolve a Editor range from a DOM event: ${event}`
            )
        }
    ...
    findTextOffsetOnLine(editor: Editor, point: Point) {
        ...
        if (!blockEntry) {
            throw new Error(
                `Cannot resolve a Editor block from a point: ${point}`
            )
        }
    ...
    findPointOnLine(
        ...
        if (!blockEntry) {
            throw new Error(
                `Cannot resolve a Editor block from a path: ${path}`
            )
        }
        const textNodes = Node.texts(blockEntry[0])
        let findOffset = 0
    ...
    toEditorPoint<T extends boolean>(
            ...
            if (leafNode) {
                ...
                if (!leafNode) {
                    ...
                    textNode = leafNode.closest(
                        `[${DATA_EDITABLE_NODE}="text"]`
                    )!
                    offsetNode = leafNode
                    offset = offsetNode.textContent!.length
                    offsetNode
                        .querySelectorAll(`[${DATA_EDITABLE_ZERO_WIDTH}]`)
                        .forEach((el) => {
        ...
        if (!textNode) {
            ...
            throw new Error(
                `Cannot resolve a Editor point from DOM point: ${domPoint}`
            )
        }
    ...
    toEditorRange<T extends boolean>(
        ...
        if (
            ...
            throw new Error(
                `Cannot resolve a Editor range from DOM range: ${domRange}`
            )
        }

        const anchor = Editable.toEditorPoint(
            editor,
