...
export const Editable = {
   ...
   * @returns
   */
  getSelectLine(editor: Editor, options: { range?: Range; match?: (element: Element) => boolean } = {}): [Element, number] | undefined {
      ...
      if (match(block)) {
        ...
        for (const rangeLine of rangeLines) {
          const index = elLines.findIndex(
            (elLine) => elLine.left === rangeLine.left && elLine.top === rangeLine.top && elLine.width === rangeLine.width
          )
          if (~index) {
            return [block, index]
          }
  ...
  findDocumentOrShadowRoot(editor: Editor): Document | ShadowRoot {
    ...
    if ((root instanceof Document || root instanceof ShadowRoot) && (root as any).getSelection != null) {
      return root
    }
  ...
  toDOMNode(editor: Editor, node: Node): HTMLElement {
    ...
    const offsetNode = Editor.isEditor(node) ? EDITOR_TO_ELEMENT.get(editor) : KEY_TO_ELEMENT?.get(Editable.findKey(editor, node))
  ...
  findLowestDOMElements(editor: Editor, node: Node) {
    ...
    const nodes = domNode.querySelectorAll(`[${DATA_EDITABLE_STRING}], [${DATA_EDITABLE_COMPOSITION}], [${DATA_EDITABLE_ZERO_WIDTH}]`)
  ...
  findClosestPoint(editor: Editor, domNode: DOMNode, x: number, y: number): Point | null {
    ...
    let element: DOMElement | null = domEl.hasAttribute(DATA_EDITABLE_NODE) ? domEl : domEl.closest(`[${DATA_EDITABLE_NODE}]`)
  ...
  findPreviousLinePoint(editor: Editor, at?: Range): Point | null {
    ...
    const startRange = Editable.toDOMRange(editor, {
      anchor: startPoint,
      focus: startPoint,
    })
    const endRange = Editable.toDOMRange(editor, {
      anchor: endPoint,
      focus: endPoint,
    })
    ...
    return Editable.findClosestPoint(editor, domBlock, isFind && !isSameLine ? startRects[0].x : 0, top)
  },

  findLineEdgePoint(editor: Editor, { at, edge = 'start' }: { at?: Range; edge?: 'start' | 'end' } = {}): Point | null {
    const { selection } = editor
    if (!at && selection) at = selection
    if (!at) return null
  ...
  findNextLinePoint(editor: Editor, at?: Range): Point | null {
    ...
    return Editable.findClosestPoint(editor, domBlock, isFind && !isSameLine ? startRects[0].x : 99999, bottom)
  },
    ...
    if (parentNode) {
      ...
      if (leafNode) {
        ...
        if (textNode) {
          ...
          const removals = [...Array.prototype.slice.call(contents.querySelectorAll(`[${DATA_EDITABLE_ZERO_WIDTH}]`))]
