import { EventTarget } from './event-emitter'
...
import { css, frames, Frames, intersects, off, on, selectAll, SelectAllSelectors, simplifyEvent } from '../utils'
...
const isTouchDevice = (): boolean => matchMedia('(hover: none), (pointer: coarse)').matches

// Determines if the browser is safari
const isSafariBrowser = (): boolean => 'safari' in window
...
export default class SelectionArea extends EventTarget<SelectionEvents> {
  ...
  /**
   * 处理开始选择的事件，如 mousedown 或 touchstart
   * @param evt
   * @param silent
   * @returns
   */
  ...
  _onSingleTap(evt: MouseEvent | TouchEvent): void {
    ...
    // 使用 simplifyEvent 函数来简化传入的事件对象，获取坐标和目标元素。
    const e = simplifyEvent(evt)

    let target

    if (intersect === 'native') {
      // 如果 intersect 设置为 'native'，则直接使用事件的目标元素。
      target = e.target
    } else if (intersect === 'touch') {
      // 如果设置为 'touch'，则在 _selectables 中查找用户点击的元素。这需要计算元素的边界，并检查点击的坐标是否在这些边界内
