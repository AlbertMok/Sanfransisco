...
export const ToolbarButton = React.forwardRef<HTMLButtonElement, ToolbarButton>(({ title, children, onToggle, active, ...props }, ref) => {
...
export const ToolbarSeparator = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>((props, ref) => (
...
export const ToolbarDropdown = React.forwardRef<HTMLButtonElement, ToolbarDropdown>(({ title, ...props }, ref) => {
  const { side } = useToolbarContext()
...
export const ToolbarSelect = React.forwardRef<HTMLInputElement, ToolbarSelect>(({ title, ...props }, ref) => {
  const { side } = useToolbarContext()

  const renderSelect = () => <Select tw="h-7 border-none hover:bg-gray-100" {...props} ref={ref} />
...
export const ToolbarColorPicker = React.forwardRef<HTMLDivElement, ToolbarColorPicker>(({ title, ...props }, ref) => {
  const { side } = useToolbarContext()
  return (
    <ColorPicker
...
const StyledToolbar = styled.div<Toolbar>(({ mode }) => [
  ...
  mode === 'inline' && tw`bg-white shadow-md z-50 px-2 py-1 rounded border-zinc-200 border-solid border`,
])
...
export const Toolbar: React.FC<Toolbar> = (props) => {
  ...
  return (
    <ToolbarContext.Provider value={{ side, mode }}>
      <StyledToolbar {...props} onClick={composeEventHandlers(props.onClick, (e) => e.preventDefault())} />
