...
import { useFloating, autoUpdate, offset, shift, limitShift, hide, arrow as floatingUIarrow, flip, Strategy } from '@floating-ui/react-dom'
...
const usePopperContext = () => {
  ...
  if (!context) throw new Error(`[usePopperContext] must be used within a ${POPPER_NAME} component.`)
...
const PopperAnchor = React.forwardRef<PopperAnchorElement, PopperAnchorProps>((props: PopperAnchorProps, forwardedRef) => {
...
const useContentContext = () => {
  ...
  if (!context) throw new Error(`[usePopperContentContext] must be used within a ${CONTENT_NAME} component.`)
...
const usePositionContext = () => {
  ...
  if (!context) throw new Error(`[usePositionContext] must be used within a ${CONTENT_NAME} component.`)
  return context
}
...
const PopperContent = React.forwardRef<PopperContentElement, PopperContentProps>((props: PopperContentProps, forwardedRef) => {
  const {
...
const PopperArrow = React.forwardRef<PopperArrowElement, PopperArrowProps>(function PopperArrow(props: PopperArrowProps, forwardedRef) {
