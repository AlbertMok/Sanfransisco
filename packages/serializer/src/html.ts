...
const withEditorHTMLSerializerTransform: HTMLSerializerWithTransform<EditorHTMLSerializerOptions> = (next, _, { editor }) => {
  return (node, options = {}) => {
...
export const HTMLSerializer = {
  ...
  create(tag: string, attributes: HTMLSerializerAttributes = {}, style?: HTMLSerializerStyle, children: string = '') {
  ...
  mergeOptions<T = HTMLSerializerAttributes | HTMLSerializerStyle>(node: Node, options: T, ...customOptions: (T | ((node: Node) => T))[]) {
    let mergedOptions = options
    for (const customOption of customOptions) {
      mergedOptions = merge(mergedOptions, customOption instanceof Function ? customOption(node) : customOption)
    }
  ...
  withEditor<T = HTMLSerializerOptions>(editor: Editor, transform: HTMLSerializerWithTransform<T>, options: T) {
...
export type HTMLSerializerAttributes =
  | Omit<Omit<React.AllHTMLAttributes<HTMLElement>, 'value'>, keyof React.DOMAttributes<HTMLElement> | 'style'>
...
const HTML_SERIALIZER_TRANSFORMS: WeakMap<Editor, EditorHTMLSerializerWithTransform[]> = new WeakMap()
